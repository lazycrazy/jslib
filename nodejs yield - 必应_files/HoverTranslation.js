HT = new function () {
    ...
    function AddSpanForWords(node)
    {
        ...
        if ((node.tagName == "SCRIPT") || node.tagName == "STYLE" || node.tagName == "CITE")
        {
            return;
        }
    ...
    function requsetTranslation()
    {
        ...
        var downloadLink = '<a href="http://dict.bing.msn.cn" target="_blank" class="ht_download">' + $config.sDeskTop + '</a>';
    ...
    this.Apply = function (word, translation)
    {
        ...
        if (translation)
        {
            ...
            var moreLink = '<h4><a href="' + $config.sMoUrl.replace("{0}", word) + '" target="_blank" onclick="HT.MoreLinkClick();">' + word + '</a></h4>';
